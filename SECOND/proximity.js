...
// Sort in term of proximity from GA

var proximity = function(startPoint, results) {

    results.forEach(loc => {
        dfs = loc["location"]["lat"] - startPoint["location"]["lat"]
        if (dfs < 0) {
            loc["distance"] = -(dfs)
        } else {
            loc["distance"] = dfs
        ...
        // console.log(results);
        // order the array by smallest to biggest
