...
// Each time you run it, you should see a random combination from the two arrays below for a great startup idea, e.g. "Skynet for Pandas"
// console.log(thisArray.length);
// console.log(thatArray.length);

function startupIdea () {
    // generates a random number
    // use that number as index to pick a spot in the array;
    //this array
    var thisI = Math.floor(Math.random() * thisArray.length);
    //that array
    var thatI = Math.floor(Math.random() * thatArray.length)
    
    // Display that
    console.log(thisArray[thisI] +" for "+ thatArray[thatI]);
};